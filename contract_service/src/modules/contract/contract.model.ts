...
class farmModel extends BaseModel {
  ...
  private async getFarm(it: any): Promise<void> {
    try {
      ...
      const { chain_id, pid, masterchef, deposit_token, token_type, address, ac_token }: any = it;
      ...
      const acPerBlock: Number = await masterChefHelper.getTokenPerBlock(ac_token, address, chain_id)
